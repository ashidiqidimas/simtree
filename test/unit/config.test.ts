...
describe("generateConfig", () => {
  it("generates .flowdeck/config.json from a repo template", () => {
    ...
    try {
        ...
        JSON.stringify(
          {
            ...
            derivedDataPath: path.join(repoRoot, ".DerivedData"),
      ...
      expect(output).toEqual({
        ...
        derivedDataPath: path.join(worktreePath, ".DerivedData"),
        xcodebuild: { args: ["-quiet"] },
      })
  ...
  it("preserves nested workspace paths in .xcodebuildmcp/config.yaml", () => {
    ...
    try {
      ...
      expect(output).toContain(`derivedDataPath: ${path.join(worktreePath, ".DerivedData")}`)
    } finally {
      fs.rmSync(repoRoot, { recursive: true, force: true })
      fs.rmSync(worktreePath, { recursive: true, force: true })
    }
  ...
  it("generates configs found inside nested app folders", () => {
    ...
    try {
      ...
      expect(flowdeckOutput.derivedDataPath).toBeUndefined()
